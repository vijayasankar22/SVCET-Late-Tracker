...
import { cleanupLastRecord } from '@/lib/actions';
import { Loader2, Trash2 } from 'lucide-react';
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from '@/components/ui/alert-dialog';

export default function CleanupLastPage() {
  const [isLoading, setIsLoading] = useState(false);
  const { toast } = useToast();

  const handleCleanup = async () => {
    setIsLoading(true);
    const result = await cleanupLastRecord();
    setIsLoading(false);
    ...
    <div className="flex min-h-screen w-full items-center justify-center bg-primary p-4">
      <Card className="w-full max-w-lg shadow-2xl">
        <CardHeader>
          ...
          <CardDescription>
            Permanently delete the single most recent late entry record from the database.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This action cannot be undone. This will permanently delete the last late entry record from your database.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
